...
static char Sccsid[] =	"@(#)cut.c	3.0	4/21/86";
/*
 *	cut -- cut and paste columns of a table (from SYSTEM V)
 */
/*  Based on:  (SYSTEM V)   cut.c	   1.3   */
...
do {	/* for all input files */
	...
	if (inptr == NULL) {
		write(2,"cut: Cannot open: ",18);
		p1=argv[filenr];	/* use p1 since we exit anyway */
		while(*p1)
			write(2,p1++,1);
		write(2,"\n",1);	/* add the <CR> */
		exit(2);
