...
static char Sccsid[] = "@(#)delta.c	3.0	4/22/86";
USXALLOC();
...
delta(file)
{
	...
	int diffloop;
	int difflim;
	char whybuff[BUFSIZ];	/* Used when why-file doesn't already exist */

	if (setjmp(Fjmp))
		return;
	sinit(&gpkt,file,1);
	if (first) {
		if (checkowner(&gpkt) == 0)
			first = 0;
		dohist(file);
	}
	...
	*/
	...
	/*
	 * remove the why-file if a normal one, else ask them
	 * for the why-file name, unless environment variable
	 * "SCCSWHY" is turned off.
	 */
	if (! had_dir)
	{
	    char whyfile[BUFSIZ];
	    strcpy(whyfile, gfilename);
	    strcat(whyfile, ".why\0");

	    if (stat(whyfile, &sbuf) >= 0) {
		if (S_ISREG(sbuf.st_mode))
			unlink(whyfile);
	    } else {
	        /*
	         * Prompt for the alternate why-file name to remove,
	         * but only if user has not turned off the SCCSWHY feature.
	         */
	        if (((p=getenv("SCCSWHY")) == NULL) ||
	            (((p=getenv("SCCSWHY")) != NULL) && (strcmp(p,"off") != 0)))
		{
		    printf("\n%s: not found\nCannot remove the standard why-file.\n", whyfile);
		    for(;;) {
		        printf("\nEnter an alternate why-file to remove, ");
			printf("<RETURN> if none:\n? ");
			if (fgets(whybuff, 132, stdin) != NULL) {
				if (strlen(whybuff) > 1) {
					...
					if (strlen(whybuff) > 14) {
						whybuff[14] = '\0';
						printf("\nCan only have 14 characters maximum.\n");
					}
					strcpy(whyfile, whybuff);
					if (stat(whyfile, &sbuf) < 0) {
						printf("\nSorry, why-file \"%s\" not found.\n", whyfile);
						continue;
					} else {  /* file exists, ok */
						if (S_ISREG(sbuf.st_mode)) {
							unlink(whyfile);
							...
							printf("\nSorry, \"%s\" is not a regular file.\n", whyfile);
					}
				} else { /* entered a <CR> only */
					printf("No why-file.\n");
					break;
				}
			} else {
				printf("\n");
				break;	/* CTRL/D, just drop it. */
			}
		    } /* forever */
		} /* end SCCSWHY */
	    } /* end "else prompt for alternate" */
	}
...
/*
 * Routine to check that the uid specified in the p-file
 * matches that of the owner.  Returns 0 if OK, otherwise
 * -1, after printing a fatal error message.
 */
checkowner(pkt)
struct packet *pkt;
{
	FILE *in;
	char *user;
	char line[BUFSIZ];	/* line from p-file */
	char fatalmess[BUFSIZ];
	int retval = 0;
	struct pfile pf;

	user = logname();	/* get username */
	zero(&pf, sizeof(pf));
	in = xfopen(auxf(pkt->p_file, 'p'), 0);
	while (fgets(line, sizeof(line), in) != NULL) {
		pf_ab(line, &pf, 0);
		if (!equal(pf.pf_user, user)) {
			sprintf(fatalmess, "login name (%s) does not match p-file (%s)", user, pf.pf_user);
			retval = fatal(fatalmess);
		}
	}
	fclose(in);
	return(retval);
