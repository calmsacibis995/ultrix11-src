...
static char Sccsid[] = "@(#)sccs.c 3.0 4/22/86";
# include <stdio.h>
# include <sys/types.h>
# include <sys/stat.h>
# include <ndir.h>
...
/* static char SccsId[] = "@(#)sccs.c	1.69 1/13/83"; */

	...
	*/
	...
	switch (cmd->sccsoper)
	{
	  ...
	  case FIX:		/* fix a delta */
		...
		p = ap[1];
		p +=2;
		if(*p < '1' || (*(p+1) == '.' && *(p+2) < '1' && *(p+3) != '.'))
		{
			printf("sccs: fix not allowed on base level %s\n", p);
			exit(1);
		}
...
# endif
	  ...
	  case ENTER:		/* enter new sccs files */
		...
		if(goodpath == 0)
		{
			if(stat(SccsPath, &tstats) == -1)
			{
				sprintf(chkbuf, "mkdir %s", SccsPath);
				system(chkbuf);
				chmod(SccsPath, 0755);
			}
			if(access(SccsPath, 7))
			{
				printf("sccs - enter: Cannot access/create %s\n",SccsPath);
/*
				exit(1);
*/
			}
	...
	char *fn;
{
	...
	*/
	...
	if (delete)
	{
		...
		/*
		 * remove filedot.why, don't bother checking if SCCSWHY=off,
		 * just clean up.
		 */
		if (strlen(fn) <= 12) {
			char whyfile[FBUFSIZ];
			strcpy(whyfile, fn);
