 ...
 **********************************************************************/
static char Sccsid[] = "@(#)run.c 3.0 4/21/86";
#include "awk.def"
#include	"math.h"
#define RECSIZE 512
#include "awk.h"
...
obj execute(u) node *u;
{
	...
	for (a = u; ; a = a->nnext) {
		...
		if (a->ntype==NPA2)
			...
			if (notlegal(a->nobj))
				error("illegal statement %o", FATAL, a);
...
obj program(a, n) node **a;
{
	...
	if (a[0] != NULL) {
		...
		if (isexit(x))
			return(true);
		if (isjump(x))
			error("unexpected break, continue or next", FATAL);
		tempfree(x);
	}
	while (getrec()) {
		x = execute(a[1]);
...
obj boolop(a,n) node **a;
{
	...
	switch (n) {
	default:
		error("unknown boolean operator %d", FATAL, n);
	case BOR:
		if (i) return(true);
		y = execute(a[1]);
		i = istrue(y);
...
obj relop(a,n) node **a;
{
	...
	switch (n) {
	default:
		error("unknown relational operator %d", FATAL, n);
...
obj gettemp()
{
	...
	for (i=0; i<MAXTMP; i++)
		if (tmps[i].tval==0)
			break;
	if (i==MAXTMP)
		error("out of temporaries in gettemp", FATAL);
	x.optr = &tmps[i];
	tmps[i] = nullval;
	x.otype = OCELL;
	x.osub = CTEMP;
...
char *format(s,a) char *s; node *a;
{
	...
	while (*s) {
		...
		*t = '\0';
		if (t >= fmt + sizeof(fmt))
			error("format item %.20s... too long", FATAL, os);
		switch (*s) {
		...
		if (a == NULL)
			error("not enough arguments in printf(%s)", FATAL, os);
		x = execute(a);
		a = a->nnext;
...
obj arith(a,n) node **a;
{
	...
	switch (n) {
	default:
		error("illegal arithmetic operator %d", FATAL, n);
	case ADD:
		i += j;
		break;
	case MINUS:
...
obj assign(a,n) node **a;
{
	...
	case MODEQ:
		...
		break;
	default:
		error("illegal assignment operator %d", FATAL, n);
		break;
	}
	tempfree(y);
	setfval(x.optr, xf);
...
obj instat(a, n) node **a;
{
	...
	arrayp = (cell *) a[1];
	if (!(arrayp->tval & ARR))
		error("%s is not an array", FATAL, arrayp->nval);
	tp = (cell **) arrayp->sval;
...
obj jump(a,n) node **a;
{
	...
	switch (n) {
	default:
		error("illegal jump type %d", FATAL, n);
		break;
	case EXIT:
		if (a[0] != 0) {
			y = execute(a[0]);
...
obj fncn(a,n) node **a;
{
	...
	if (t == FLENGTH)
		...
		error("illegal function type %d", FATAL, t);
	tempfree(x);
	x = gettemp();
	setfval(x.optr, u);
	return(x);
...
obj print(a,n) node **a;
{
	...
	if (strlen(s) >= RECSIZE)
		error("string %.20s ... too long to print", FATAL, s);
	if (a[1]==nullstat) {
		printf("%s", s);
		return(true);
	}
...
redirprint(s, a, b) char *s; node *b;
{
	...
	for (i=0; i<FILENUM; i++)
		if (files[i].fp == 0)
			break;
	if (i >= FILENUM)
		error("too many output files %d", FATAL, i);
	if (a == '|')	/* a pipe! */
	...
	if (files[i].fp == NULL)
		error("can't open file %s", FATAL, x.optr->sval);
