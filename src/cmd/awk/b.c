 ...
 **********************************************************************/
static char Sccsid[] = "@(#)b.c 3.0 4/21/86";
#include "awk.def"
#include "stdio.h"
#include "awk.h"
...
node *p;
{
	switch(type(p)) {
		...
		case OR:
			...
			break;
		default:
			error("unknown type %d in penter\n", FATAL, type(p));
			break;
	}
}
...
node *p;
{
	switch(type(p)) {
		...
		case OR:
			...
			break;
		default:
			error("unknown type %d in freetr", FATAL, type(p));
			break;
	}
}
char *cclenter(p)
...
register node *v;
{
	...
	switch(type(v)) {
		...
		case OR:
			...
			break;
		default:
			error("unknown type %d in cfoll", FATAL, type(v));
	}
}
