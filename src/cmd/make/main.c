...
static char Sccsid[] = "@(#)main.c	3.1	8/25/87";
# include "defs"
/*
...
char ftlstr[] = "No arguments or description file";
char junkname[20];
char funny[128];
...
for(i=1; i<argc; ++i)
	...
	for(j=1 ; (c=argv[i][j])!='\0' ; ++j)  switch(c)
		...
		case 'f':
			...
			if( rddescf(argv[i+1]) ){
			      sprintf(tbuf,"/usr/bin/sccs get %s",argv[i+1]);
				system(tbuf);
				if( rddescf(argv[i+1]) )
...
if( !descset )
	...
	{
		if(access("./SCCS/s.makefile", 0) == 0)
		{
			system("/usr/bin/sccs get makefile");
			if(rddescf("makefile"))
			{
				if(access("./SCCS/s.Makefile", 0) == 0)
				{
		...
		else if(access("./SCCS/s.Makefile", 0) == 0)
		{
			system("/usr/bin/sccs get Makefile");
			if(rddescf("Makefile"))
				fatal(ftlstr);
