...
static char Sccsid[] = "@(#)lpass2.c	3.0	4/21/86";
# include "lmanifest"
# include "manifest"
...
lastone(){
	...
	if( !ISFTN(pd->type.aty) ){
		...
		case LUV:
		case LUE:
		case LUV | LUE:		/* (SYSTEM V) or'ed for void */
		case LUM:
			nd = 1;
			}
		}
