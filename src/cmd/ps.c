 ...
 *
 *	Fred Canter 10/16/82
 */

static char Sccsid[] = "@(#)ps.c 3.1 7/1/87";
#include <stdio.h>
#include <a.out.h>
#include <core.h>
#include <sys/param.h>
...
char	devdn[30];	/* convert names in /dev directory to `/dev/name' */
...
bbreak:
/* This caused problems for CDA, required full pathnames.
 * Changed getdev(), added /dev to names.
	if(chdir("/dev") < 0) {
...
#include <sys/stat.h>
	...
	while (fread((char *)&dbuf, sizeof(dbuf), 1, df) == 1) {
		if(dbuf.d_ino == 0)
			continue;
		sprintf(&devdn, "/dev/%s", dbuf.d_name);
		if(stat(devdn, &sbuf) < 0)
			continue;
		if ((sbuf.st_mode&S_IFMT) != S_IFCHR)
			continue;
/*
 * The /dev/dh, uh, dz, dl character mode special files
 * are used by the CMX on-line comm. device exerciser.
 * These files confuse ps so they are ignored.
 */
		if((strncmp("dh", dbuf.d_name, 2) == 0) ||
		   (strncmp("uh", dbuf.d_name, 2) == 0) ||
		   (strncmp("dl", dbuf.d_name, 2) == 0) ||
			(strncmp("dz", dbuf.d_name, 2) == 0))
				continue;
...
prcom(puid)
{
	...
	if (tptr && strncmp(tptr, tp, 2))
		return(0);
	if (lflg) {
		printf("%4o %c%4d", mproc.p_flag,
			"0SWRIZT"[mproc.p_stat], puid);
	}
	printf("%6u", mproc.p_pid);
	if (lflg) {
		printf("%6u%4d%4d%5d", mproc.p_ppid, mproc.p_cpu&0377,
			mproc.p_pri,
			mproc.p_nice);
		if(mproc.p_flag&SLOAD)
			printf("%7o%4d", mproc.p_addr, (mproc.p_size+7)>>3);
		else
			printf("%7d%4d", (mproc.p_addr-1), (mproc.p_size+7)>>3);
		if (mproc.p_wchan)
