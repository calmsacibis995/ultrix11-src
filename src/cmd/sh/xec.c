...
#
static char Sccsid[] = "@(#)xec.c	3.0	4/22/86";
/*
 * UNIX shell
 *
 * S. R. Bourne
		...
		case TCOM:
			...
			THEN	/* print command if execpr */
				...
				case SYSCONT:
					...
					break;
			/* V7 SYSCONT:	execbrk = -loopcnt; break; */
	
				case SYSBREAK:		/* using SYSV version */
					if (loopcnt)
					{
						execbrk = breakcnt = 1;
						if (a1)
...
*/
		...
		case TFOR:		/* for.. do */
			BEGIN
			   ...
			   DO	assign(n,*args++);
				execute(t->fortre,0);
		/* V7:		IF execbrk<0 THEN execbrk=0 FI	*/
				if (breakcnt < 0)		     /* SYSV */
					execbrk = (++breakcnt != 0); /* SYSV */
			   OD
		/* V7:	   IF breakcnt THEN breakcnt-- FI	*/
		/* V7:	   execbrk=breakcnt;			*/
			   if (breakcnt > 0)			/* SYSV */
				execbrk = (--breakcnt != 0);	/* SYSV */
			   loopcnt--;
