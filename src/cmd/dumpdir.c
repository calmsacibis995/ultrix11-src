...
static char Sccsid[] = "@(#)dumpdir.c 3.1 10/9/87";

/* Added the 'd' option to list the /etc/ddate file
 * 
 * 512dumpdir: compile by defining REST_512 (see dumprestor.h)
 * 	used for reading tapes written from 512-byte block file systems
...
char *argv[];
{
	...
	if (argc==2 && *argv[0] == 'd')   /* list the ddate file */
		prdate();
	mktemp(dirfile);
	if (argc>=3 && *argv[0] == 'f')
		magtape = *++argv;
...
prdate()
{
	...
	fd = open("/etc/ddate", 0);
	if(fd < 0) {
		printf("\ncannot open ddate file!\n");
		exit(1);
	}
	printf("\nDisk\tLevel\tDump Date");
	printf("\n----\t-----\t---------\n");
	while(1) {
		cc = read(fd, (char *)&idbuf, sizeof(idbuf));
		if(cc == 0)
			exit(0);
		if(cc < 0) {
			printf("\nread error!\n");
		...
		printf("%s\t%c\t%s", idbuf.id_name, idbuf.id_incno,
			ctime(&idbuf.id_ddate));
	}
	exit(0);
}
