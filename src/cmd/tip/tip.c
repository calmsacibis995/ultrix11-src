...
#ifndef lint
static char sccsid[] = "@(#)tip.c	3.1	1/13/88";
#endif
...
cucommon:
	...
	ioctl(0, TIOCGETV, (char *)&defchars);	/* get vmin & vtime */
#ifdef TIOCGLTC
...
unraw()
{
	...
	ioctl(0, TIOCSETV, (char *)&defchars);	/* set vmin & vtime */
#ifdef TIOCSLTC
 ...
 * Fred Canter -- 1/13/88
 * Don't need to set vmin/vtime becasue they are
 * set to 1/0 by the system in RAW mode.
 */
ttysetup(speed)
	int speed;
{
