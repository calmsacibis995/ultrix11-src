...
static char *Sccsid = "@(#)sh.exec.c	3.0	4/21/86";
/*
 * Base on
 *	char *sccsid = "@(#)sh.exec.c 4.1 10/9/80";
 */
	...
	register char **t;
{
	...
	case ENOMEM:
		/* 
		 * NOTE: large processes on small memory sized machines
		 * (<=256KB) could hang, so we perror and exit (child)
		 * instead of called Perror(f).
		 */
		/* Perror(f); */
		perror(f);
		exit(1);
