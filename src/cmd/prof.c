...
static char Sccsid[] = "@(#)prof.c 3.0 4/22/86";
#include <stdio.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <a.out.h>
...
char **argv;
{
	...
	if (xbuf.a_magic!=A_MAGIC1 && xbuf.a_magic!=A_MAGIC2
	   && xbuf.a_magic!=A_MAGIC3 && xbuf.a_magic!=A_MAGIC6
	   && xbuf.a_magic!=A_MAGIC7) {
		fprintf(stderr, "%s: bad format\n", namfil);
		done();
	}
	if(xbuf.a_magic == A_MAGIC6 || xbuf.a_magic == A_MAGIC7){
		fread(ovhdr, 1, sizeof(ovhdr), nfile);
		symoff = (long)xbuf.a_text;
		for(i = 1; i < 8; i++)
			symoff += (long)ovhdr[i];
		symoff += (long)xbuf.a_data;
	}
	else
		symoff = (long)xbuf.a_text + xbuf.a_data;
	if ((xbuf.a_flag&01) == 0)
		symoff *= 2;
	symoff += (long)sizeof(xbuf);
	if(xbuf.a_magic >= 0430)
		symoff += (long)sizeof(ovhdr);
