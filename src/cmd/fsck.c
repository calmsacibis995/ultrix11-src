...
/*
 * Based on @(#)fsck.c	4.13 (Berkeley) 81/03/09"
 */
static char sccsid[] = "@(#)fsck.c	3.0	4/21/86";

/* Changes made to recognize FIFO files
...
char *s;
{
	...
	if (startpass <= 0 || startpass > endpass)
		errexit("-p: invalid pass numbers\n");
}
...
char *dev;
{
	...
	if((statarea.st_mode & S_IFMT) == S_IFBLK)
		...
		pfatal("file is not a block or character device");
		if (reply("OK") == NO)
			return(NO);
	}
...
char *name;
{
	if (preen == 0)
		printf("\n");
	pfatal("CAN NOT %s: BLK %ld of %s",s,blk,name);
	if(reply("CONTINUE") == NO)
...
ifreechk() {
	...
	if (superblk.s_ninode < 0 || superblk.s_ninode > NICINOD) {
		pwarn("BAD FREE INODE COUNT");
		if (preen)
			printf(" (FIXED)\n");
		if (preen || reply("FIX") == YES) {
			superblk.s_ninode = 0;
			superblk.s_nbehind = 0;
			superblk.s_lasti = 1;
			sbdirty();
			return;
		}
	...
	for (i=0; i<superblk.s_ninode; i++) {
		/*
		 * The kernel ignores inodes < ROOTINO, so we
		 * will too.  This means we don't complain when
		 * the kernel sticks inode 1 into the free list.
		 */
		if ((inum = superblk.s_inode[i]) < ROOTINO)
			continue;
		switch (getstate()) {
...
char *buf;
{
	if(lseek(fcp->rfdes,blk<<BSHIFT,0) < 0)
		rwerr("SEEK",blk,fcp->fname);
	else if(read(fcp->rfdes,buf,size) == size)
		return(YES);
	rwerr("READ",blk,fcp->fname);
	return(NO);
}
