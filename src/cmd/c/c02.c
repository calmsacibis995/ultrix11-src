...
static char *Sccsid = "@(#)c02.c	3.0	(ULTRIX)	4/21/86";
#include "c0.h"
...
cfunc()
{
	...
	maxauto = STAUTO;
#else	!SYSTEM_V
	autolen = (ovflg ? STAUTOOV : STAUTO);
	maxauto = (ovflg ? STAUTOOV : STAUTO);
#endif	SYSTEM_V
	blklev = 1;
	cb = locbase;
	declist(ARG);
	 ...
	 * subtract off size of stack fram here,
	 * instead of in second pass.
	 * c11.c also changed.
	 */
	outcode("BN", SETSTK, (-maxauto) + (ovflg ? STAUTOOV : STAUTO));
#endif	SYSTEM_V
	branch(sloc+1);
	locbase = cb;
}
...
register struct nmlist *ocs, *cs;
{
	...
	for (; ocs!=NULL; ocs = ocs->nextnm) 
		if (ocs!=cs && ocs->hclass==EXTERN && strncmp(cs->name, ocs->name, 7) == 0)
			error("names %.8s and %.8s conflict", cs->name, ocs->name);
}
