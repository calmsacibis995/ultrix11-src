...
static char *Sccsid = "@(#)c12.c	3.0	(ULTRIX)	4/21/86";
#include "c1.h"
...
register union tree *tree;
{
	...
	case FSELA:
		/*
		 * If we are assigning to an unsigned char bit field,
		 * treat it as if it were a character bit field.
		 *	-Dave Borman, 11/5/85
		 */
		if (tree->t.tr1->t.type==UNCHAR)
			tree->t.tr1->t.type=CHAR;
...
register union tree *tree;
{
	...
	if (op==PLUS || op==OR) {
		...
		if (t1->t.op==CON) {
			if (t1->c.value==0 && (acl.nextl<=0 || !sideeffects(t2[-1])))
				return(t1);
