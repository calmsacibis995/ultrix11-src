...
#ifdef KEEPSCCSID
static char Sccsid[] = "@(#)io.c	3.0	4/21/86";
#endif
...
register struct Addrblock *unitp, *nump, *fmtp, *recp, *tioblkp;
...
if(intfile)
	...
	ioset(TYIOINT, XUNIT, unitp);

if(recp)
	ioset(TYIOINT, (intfile ? XIREC : XREC) , recp);
...
if(statstruct)
	...
	dataline(dataname(STGINIT,ioblkp->memno), (ftnint) offset,
		blklen, TYADDR);
	if(p)
		praddr(initfile, p->vstg, p->memno, p->memoffset->const.ci);
	else
