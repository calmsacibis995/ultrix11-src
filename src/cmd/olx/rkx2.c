...
static char Sccsid[] = "@(#)rkx2.c	3.0	4/22/86";
/*
...
int argc;
{
	...
	if((argc < 2) || (argc > 4))
		exit(1);
/*
 * Read needed date from the `rkx_#.arg' file (# = drive).
 */
	argf = fopen(argv[1], "r");
	if(argf == NULL)
		exit(1);
 ...
 */
	...
	if(bn_rw >= RKNBLK)
		bn_rw = RKNBLK - 1;
	while((bn_rw + ((nbytes + 511) / 512)) > RKNBLK)
		bn_rw -= 1;
	bn_rr = rng() & 017777;
	while(bn_rr >= RKNBLK)
		bn_rr <<= 1;
...
e_erc:	/* Entry point for hard error xfer restart */
	if(fswrs == 0) {
		...
		if((rbc = write(efd, (char *)wbp, nbytes)) != nbytes)
			dskse((cnt | 010), dn, 0, bn_rw, nbytes);
		}
/*
...
next:
	if(++count >= NPASS) {
		...
		if((i = fork()) > 0)
			exit(0);
		if(i == -1)
			fprintf(stderr,"\nrkx: Can't fork new copy of rkx !\n");
	}
	if(sflag)
		goto e_loop;
	time(&timbuf);
...
fsopen(dn, fso)
{
	...
	if(fd_rw[j] < 0) {
		if(fso < 8)			/* generate file name */
			sprintf(&fn, "/dev/rk%o", dn);
		else
			sprintf(&fn, "/dev/rrk%o", dn);
