...
static	char Sccsid[] = "@(#)xstr.c	3.0	4/22/86";
...
flushsh()
{
	...
	if (new == 0 && old != 0)
		return;
#ifdef	doesntworkwithsystem5
	mesgwrit = fopen(strings, old ? "a" : "w");
#else
	/*
	 * "a" makes everything go to the end, and "w" clobbers
	 * the file. We really need to be able to open the file
	 * "m", for modify, but...
	 */
