...
static char *Sccsid = "@(#)get_names.c	3.0	(ULTRIX-11)	4/22/86";
...
char *argv[];
{
    ...
    /*
     * See if we have networking in the kernel.  The listen()
     * call should always fail, and by looking at errno we'll
     * know whether or not we have networking.  EINVAL indicates
     * no networking, EBADF means we do.
     */
...
#include <errno.h>
	if (errno != EINVAL)
	    ...
	    printf("Sorry, talk requires kernel network support.\n");
	    exit(-1);
	}
    }
