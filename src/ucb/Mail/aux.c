...
#ifndef	lint
static char 	*Sccsid = "@(#)aux.c	3.1	12/3/87";
#endif
	...
	register int rem;
{
	...
	for (;;) {
		...
		for (;;) {
			...
			/* rjg.. Remove check for lf. Causes subject
			 * to be ommitted on header command if an
			 * extra line feed is included before/after
