...
#ifndef lint
static char *Sccsid = "@(#)collect.c	3.0	4/22/86";
#endif
 ...
 * ~ escapes.
 *
 *	29-May-84	ma.  Add message text prompt.
 *
 *	18-Jun-84	ma.  Change <ctrl> to <CTRL/*>.
 *
 */
	...
	struct header *hp;
{
	...
	struct tchars special;		/*29-May-84 addition*/
...
# endif VMUNIX
	...
	if (intty && sflag == NOSTR && hp->h_subject == NOSTR && value("ask"))
		t &= ~GNL, c++;
						/*29-May-84 addition*/
	if (intty && value("msgprompt"))
		t &= ~GNL;			/*if mssprompt is set, we never
						 *want to put our a newline.
						 */
						/*end of addition*/
	if (hp->h_seq != 0) {
		puthead(hp, stdout, t);
	...
	if (c)
		grabh(hp, GSUBJECT);
						/*29-May-84 addition*/
	if (intty && value("msgprompt")) {
		ioctl(0,TIOCGETC,&special);	/*get special characters*/
		printf("Enter message. Use ");
		if (value("ignoreeof") == NOSTR)/*if not ignoring eof then
						 *always state eof for
						 *termination.
						 */
			if (special.t_eofc < 0x20)
				printf("<CTRL/%c> ",special.t_eofc|0x40);
			else
				printf("%c ",special.t_eofc);
		if (value("ignoreeof") == NOSTR && value("dot") != NOSTR)
						/*not ignoring eof and dot is in
						 *use, therefore can use either.
						 */
			printf("or a period alone on a line ");
		if (value("ignoreeof") != NOSTR)/*no matter the setting of dot,
						 *if ignoring eof, then we are
						 *using dot to terminate.
						 */
			printf("a period alone on a line ");
		printf("to terminate the letter.\n");
	}					/*end of addition*/
	escape = ESCAPE;
