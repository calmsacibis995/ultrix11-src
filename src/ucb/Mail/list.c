...
#ifndef lint
static char *sccsid = "@(#)list.c	3.0	4/22/86";
#endif
/*
 * list.c
 *
 *	sccsid[] = "@(#)list.c	2.3 (Berkeley) 8/11/83";
 *
 *	7/4/84		-- gpc.  added code to skip deleted messages on  
 *			'+' and '-' commands
 */
...
number:
		...
		case TPLUS:
			...
			/*
			 * 5-Jul-84 gpc. Skip over deleted messages
			 */
			while ( (message[valdot].m_flag & MDELETED) == MDELETED) {
				if (valdot >= msgCount)
					break;
				valdot++;
			 }
		...
		case TDASH:
			if (beg == 0) {
				 ...
				 * 5-Jul-84 gpc. Skip over deleted messages
				 */ 
				while ( (message[valdot-2].m_flag & MDELETED) == MDELETED)  {
					if (valdot <= 2) 
						break;
					valdot--;
				}
