...
/*
 ...
 Author:	Unknown - Part of the original V7 plot package from Bell.

 Creation:	Unknown - somewhere back in the mists of time...1970s
 ...
 24-Jun-1984	Kevin J. Dunlap  Remove map and colour calls, LA100 does not
				 use them.
*/
#include <stdio.h>
#define DEBUG 0 		/* DaveR - add debug output ability */
float deltx;
float delty;
...
fplt(fin)  FILE *fin; {
	...
	while((c=getc(fin)) != EOF){
		...
		case 'd':			/* d - dot */
			...
			break;
		case 'j':	/* DaveR - set the colour to write in */
			xi = getsi(fin);	/* get the colour no */
/*			colour(xi);		/* select it */
			break;
		case 'k':	/* DaveR - load a colour map register */
