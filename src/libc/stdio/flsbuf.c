...
/*
 * SCCSID: @(#)flsbuf.c	3.0	4/22/86
 */
/*	(System 5)  flsbuf.c	2.8	*/
/*LINTLIBRARY*/
#include <stdio.h>
#include "stdiom.h"
#include <sys/errno.h>
...
{
	/*
	 * Rather than checking if writing in enabled,
	 * check for a read-only stream, since this is the
	 * only legitimate case where it does not make sense
	 * to call fflush().
	 */
/*	if (!(iop->_flag & _IOWRT)) { 	 SYSTEM V original */
	if ((iop->_flag & (_IORW|_IOREAD)) == _IOREAD) {
	/*
	 * The next 2 lines were originally part of SYSTEM V, but
	 * were taken out for ULTRIX-11 V2.0 compatibililty.
	 * The iop->_cnt = 0 causes a subsequent ftell() to return
	 * ptr to EOF, rather than keep it's current location.
	 * 	-John Dustin
	 */
	/*	iop->_cnt = 0;	SYSTEM V only */
	/*	return(0);	SYSTEM V only */
	/* setting error bit in next line breaks ucbMail */
	/*	iop->_flag |= _IOERR;	/* added */
	/* EOF in next line breaks all F77 programs */
	/*	return(EOF);		/* added */
	/* therefor, we just return(0) like before */
		return(0);
	}
